...
void GraphicsSystem::draw(GraphicsBuffer* ptbuffer, Vector2D tLoc, Sprite spr, float scale)
{
	...
	al_draw_scaled_bitmap(spr.getGraphicsBuffer()->getBitmap(), spr.getSourceLoc().getX(), spr.getSourceLoc().getY(), spr.getWidth(), spr.getHeight(), tLoc.getX(), tLoc.getY(), al_get_bitmap_width(spr.getGraphicsBuffer()->getBitmap()) * scale, al_get_bitmap_height(spr.getGraphicsBuffer()->getBitmap()) * scale, 0);
}
